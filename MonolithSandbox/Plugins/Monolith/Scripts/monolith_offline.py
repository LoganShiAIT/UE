...
class SourceActions:
    ...
    @classmethod
    def _contains_token(cls, line, token):
        start = 0
        while start < len(line):
            ...
            if ((at == 0 or not cls._is_identifier_char(line[at - 1]))
                    and (end >= len(line) or not cls._is_identifier_char(line[end]))):
                return True
            start = end
        return False
    ...
    def _is_header_path(path):
        return (path or "").lower().endswith((".h", ".hpp", ".inl"))

    @staticmethod
    def _header_path_rank(path):
    ...
    @classmethod
    def _line_declares_owner(cls, raw, owner):
        line = (raw or "").strip()
        if (not line or line.startswith(("//", "/*", "*"))
                or not cls._contains_token(line, owner)):
            return False
        return any(keyword in line for keyword in ("namespace ", "class ", "struct ", "enum "))
    ...
    def resolve_header_file(self, symbol):
        owner = symbol.rsplit("::", 1)[0] if "::" in symbol else symbol
        best = None
        rows = self.db.execute(
        ...
        for row in rows:
            path = row["path"]
            if not self._is_header_path(path):
                continue
            candidate = (self._header_path_rank(path), row["file_id"], path)
        ...
        suffixes = [owner_leaf + ".h"]
        if len(owner_leaf) > 1 and owner_leaf[0] in "FUASIET":
            suffixes.append(owner_leaf[1:] + ".h")
        for suffix in suffixes:
            candidates = self.db.execute(
                "SELECT id, path FROM files WHERE path LIKE ? LIMIT 20", ("%" + suffix,)).fetchall()
            for candidate in candidates:
                path = candidate["path"]
                if not self._is_header_path(path):
                    continue
                if not any(self._line_declares_owner(line, owner) for line in self._load_lines(path)):
                    continue
                return candidate["id"], path

        chunks = self.db.execute(
            "SELECT file_id FROM source_fts WHERE source_fts MATCH ? "
            "ORDER BY bm25(source_fts) LIMIT 256", (escape_fts(owner),)).fetchall()
        seen = set()
        for chunk in chunks:
            ...
            if not any(self._line_declares_owner(line, owner) for line in self._load_lines(path)):
                continue
            candidate = (self._header_path_rank(path), file_id, path)
            if best is None or candidate[0] > best[0]:
                best = candidate
        return (best[1], best[2]) if best else None

    def resolve_symbol_row(self, symbol):
        lookup = symbol.rsplit("::", 1)[0] if "::" in symbol else symbol
        row = self.db.execute(
            "SELECT id, name, file_id FROM symbols WHERE name = ? ORDER BY (line_end > line_start) DESC",
            (lookup,)).fetchone()
        if row:
            return row
        resolved = self.resolve_header_file(symbol)
        return {"id": 0, "name": lookup, "file_id": resolved[0]} if resolved else None
    ...
    @classmethod
    def _is_likely_function_declaration(cls, raw, method, owner, is_header):
        line = (raw or "").strip()
        if not line or line.startswith(("//", "/*", "*", "#")):
            return False
        needle = method + "("
        at = line.find(needle)
        if at < 0 or (at > 0 and cls._is_identifier_char(line[at - 1])):
            return False
        prefix = line[:at].strip()
        if ("=" in prefix or "->" in prefix or prefix.endswith(".")
                or prefix.startswith(("return ", "if ", "if(", "for ", "for(",
                                      "while ", "while(", "switch ", "switch(",
                                      "ensure", "check"))):
            return False
        if owner:
            qualified = owner + "::" + needle
            qualified_at = line.find(qualified)
            if qualified_at >= 0:
                return bool(line[:qualified_at].strip()) or owner == method
            return is_header and (bool(prefix) or owner == method)
        return bool(prefix)

    def _collect_signatures(self, symbol, limit):
        scope = symbol.rfind("::")
        owner = "" if scope < 0 else symbol[:scope]
        method = symbol if scope < 0 else symbol[scope + 2:]
        overloads, seen = [], set()

        def add(sig, source, path, line):
            sig = (sig or "").strip()
            if not sig or sig in seen or len(overloads) >= limit:
                return
            seen.add(sig)
            overloads.append((sig, source, path, line))

        if scope < 0:
            rows = self.db.execute(
                "SELECT signature, kind, file_id, line_start FROM symbols WHERE name = ?",
                (symbol,)).fetchall()
            for row in rows:
                sig = row["signature"] or ""
                if row["kind"] == "function" or not sig:
                    continue
                add(sig, "column", self.get_file_path(row["file_id"]), row["line_start"])
            if overloads:
                return overloads

        resolved = self.resolve_header_file(symbol if scope < 0 else owner)
        if resolved:
            _file_id, path = resolved
            lines = self._load_lines(path)
            if scope < 0:
                for i, line in enumerate(lines):
                    if self._line_declares_owner(line, symbol):
                        add(line.split("{", 1)[0], "declaration_read", path, i + 1)
            else:
                for i, line in enumerate(lines):
                    if not self._is_likely_function_declaration(line, method, owner, True):
                        continue
                    sig = self.compact_declaration(lines, i)
                    if method + "(" in sig:
                        add(sig, "declaration_read", path, i + 1)
        ...
        chunks = self.db.execute(
            ...
            "ORDER BY bm25(source_fts) LIMIT 100", (escape_fts(symbol),)).fetchall()
        for chunk in chunks:
            if len(overloads) >= limit:
                break
            path = self.get_file_path(chunk["file_id"])
            lines = self._load_lines(path)
            start = max(0, chunk["line_number"] - 1)
            end = min(len(lines), start + 10)
            for i in range(start, end):
                if not self._is_likely_function_declaration(
                        lines[i], method, owner, self._is_header_path(path)):
                    continue
                sig = self.compact_declaration(lines, i)
                if method + "(" in sig:
                    add(sig, "declaration_read", path, i + 1)
    ...
    def get_signature(self, args):
        symbol = args.symbol
        limit = args.limit
        overloads = self._collect_signatures(symbol, limit)
        ...
        for sig, source, fp, line in overloads:
            out.append(f"{sig}\n  // {source} @ {self.short_path(fp)}:{line}")
        print("\n".join(out))
    ...
    def symbol_exists(self, symbol):
        scope = symbol.rfind("::")
        if scope >= 0:
            return bool(self._collect_signatures(symbol, 1))
        if self.db.execute("SELECT id FROM symbols WHERE name = ? LIMIT 1", (symbol,)).fetchone():
            return True
        return self.resolve_header_file(symbol) is not None

    def first_signature(self, symbol):
        signatures = self._collect_signatures(symbol, 1)
        return (signatures[0][0], signatures[0][1]) if signatures else (None, None)
    ...
    def find_example_usage(self, args):
        ...
        if scope >= 0:
            method = symbol[scope + 2:]
        is_type = is_function = False
        if scope < 0:
            row = self.db.execute("SELECT kind FROM symbols WHERE name = ? LIMIT 1", (symbol,)).fetchone()
            if row:
                is_type = row["kind"] in ("class", "struct", "enum")
                is_function = row["kind"] == "function"
        needle = (method + "(" if scope >= 0 else
                  (symbol + "(" if is_function else (symbol if is_type else symbol + "::")))
        ...
        for ch in chunks:
            ...
            for i in range(ws, we):
                ...
                if scope >= 0:
                    owner = symbol[:scope]
                    if self._is_likely_function_declaration(
                            fl[i], method, owner, self._is_header_path(fp)):
                        continue
                elif is_function:
                    if self._is_likely_function_declaration(
                            fl[i], symbol, "", self._is_header_path(fp)):
                        continue
                elif self._line_declares_owner(fl[i], symbol):
                    continue
                ...
                norm = fp.replace("\\", "/")
                is_engine = "Engine/Source/" in norm or "Engine/Plugins/" in norm
