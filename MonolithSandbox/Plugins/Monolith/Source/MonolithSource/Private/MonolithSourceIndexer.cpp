...
void FMonolithSourceIndexer::DiscoverEngineModules(TArray<FModuleEntry>& OutModules)
{
	...
	// Engine plugins — index each real UBT module under a plugin Source/ tree.
	//
	// The old implementation registered the plugin Source/ root as one module
	// named after the plugin directory (for example all Niagara, NiagaraEditor,
	// NiagaraCore... files became module "Niagara").  That made include/module
	// answers and Build.cs guidance incorrect.  A *.Build.cs file is the canonical
	// module boundary, so discover those and index their containing directories.
	// SourcePath is Engine/Source, parent is Engine/.
	FString PluginsDir = FPaths::GetPath(SourcePath) / TEXT("Plugins");
	TSet<FString> SeenPluginBuildCs;
	TSet<FString> SeenLooseSourceRoots;
	TArray<FModuleEntry> PluginModules;
	TArray<FModuleEntry> LooseSourceModules;
	...
	IFileManager::Get().IterateDirectoryRecursively(*PluginsDir, [&](const TCHAR* Path, bool bIsDir) -> bool
	{
		...
		{
			const FString SourceDir(Path);
			const FString PluginDir = FPaths::GetPath(SourceDir);

			TArray<FString> BuildCsFiles;
			IFileManager::Get().FindFilesRecursive(BuildCsFiles, *SourceDir, TEXT("*.Build.cs"),
				/*Files=*/true, /*Directories=*/false, /*bClearFileNames=*/true);

			for (const FString& BuildCsPath : BuildCsFiles)
			{
				FString NormalizedBuildCs = BuildCsPath;
				FPaths::NormalizeFilename(NormalizedBuildCs);
				if (SeenPluginBuildCs.Contains(NormalizedBuildCs))
				{
					continue;
				}
				SeenPluginBuildCs.Add(NormalizedBuildCs);

				FString ModuleName = FPaths::GetCleanFilename(BuildCsPath);
				ModuleName.RemoveFromEnd(TEXT(".Build.cs"));
				const FString ModuleDir = FPaths::GetPath(BuildCsPath);
				PluginModules.Add({ ModuleDir, ModuleName, TEXT("Plugin"), BuildCsPath });
			}

			// Preserve full Source coverage for third-party headers, templates and
			// disabled/legacy modules that have no Build.cs. These roots are appended
			// after all real modules; IndexModule's path de-duplication means this pass
			// indexes only files not already claimed by a real UBT module.
			FString NormalizedSourceDir = SourceDir;
			FPaths::NormalizeFilename(NormalizedSourceDir);
			if (!SeenLooseSourceRoots.Contains(NormalizedSourceDir))
			{
				SeenLooseSourceRoots.Add(NormalizedSourceDir);
				const FString LooseName = FPaths::GetCleanFilename(PluginDir) + TEXT("_LooseSource");
				LooseSourceModules.Add({ SourceDir, LooseName, TEXT("Plugin"), FString() });
			}
		}
		return true;
	});

	OutModules.Append(PluginModules);
	OutModules.Append(LooseSourceModules);
...
void FMonolithSourceIndexer::IndexModule(const FModuleEntry& Module, FMonolithSourceDatabase& DB)
{
	...
	for (const FString& FilePath : Files)
	{
		...
		if (IndexedSourcePaths.Contains(NormalizedPath)) continue;
		IndexedSourcePaths.Add(NormalizedPath);
