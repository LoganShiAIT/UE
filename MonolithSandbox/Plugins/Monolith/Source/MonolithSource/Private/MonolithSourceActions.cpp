...
namespace MonolithSourceQueryFix
{
	struct FResolvedSignature
	{
		FString Signature;
		FString Source;
		FString FilePath;
	...
	static bool ContainsToken(const FString& Line, const FString& Token)
	{
		int32 SearchFrom = 0;
		while (SearchFrom < Line.Len())
		{
			const int32 Idx = Line.Find(Token, ESearchCase::CaseSensitive, ESearchDir::FromStart, SearchFrom);
			if (Idx == INDEX_NONE) return false;
			const bool bLeftOk = Idx == 0 || !IsIdentifierChar(Line[Idx - 1]);
			const int32 End = Idx + Token.Len();
			const bool bRightOk = End >= Line.Len() || !IsIdentifierChar(Line[End]);
			if (bLeftOk && bRightOk) return true;
			SearchFrom = Idx + Token.Len();
		}
		return false;
	}

	static bool IsHeaderPath(const FString& Path)
	{
		return Path.EndsWith(TEXT(".h"), ESearchCase::IgnoreCase)
			|| Path.EndsWith(TEXT(".hpp"), ESearchCase::IgnoreCase)
			|| Path.EndsWith(TEXT(".inl"), ESearchCase::IgnoreCase);
	}

	static int32 HeaderPathRank(const FString& Path)
	{
		FString Norm = Path;
		Norm.ReplaceInline(TEXT("\\"), TEXT("/"));
		if (Norm.Contains(TEXT("/Public/"), ESearchCase::IgnoreCase)) return 400;
		if (Norm.Contains(TEXT("/Classes/"), ESearchCase::IgnoreCase)) return 350;
		if (Norm.Contains(TEXT("/Internal/"), ESearchCase::IgnoreCase)) return 300;
		if (Norm.Contains(TEXT("/Private/"), ESearchCase::IgnoreCase)) return 100;
		return 200;
	}

	static bool LineDeclaresOwner(const FString& RawLine, const FString& Owner)
	{
		const FString Line = RawLine.TrimStartAndEnd();
		if (Line.IsEmpty() || Line.StartsWith(TEXT("//")) || Line.StartsWith(TEXT("/*"))
			|| Line.StartsWith(TEXT("*")) || !ContainsToken(Line, Owner))
		{
			return false;
		}

		return Line.Contains(TEXT("namespace ")) || Line.StartsWith(TEXT("namespace"))
			|| Line.Contains(TEXT("class ")) || Line.StartsWith(TEXT("class"))
			|| Line.Contains(TEXT("struct ")) || Line.StartsWith(TEXT("struct"))
			|| Line.Contains(TEXT("enum ")) || Line.StartsWith(TEXT("enum"));
	}

	static bool ResolveHeaderFile(FMonolithSourceDatabase* DB, const FString& Symbol,
		int64& OutFileId, FString& OutFilePath)
	{
		OutFileId = 0;
		OutFilePath.Empty();
		if (!DB) return false;

		FString Owner = Symbol;
		const int32 ScopeIdx = Symbol.Find(TEXT("::"), ESearchCase::CaseSensitive, ESearchDir::FromEnd);
		if (ScopeIdx != INDEX_NONE) Owner = Symbol.Left(ScopeIdx);

		// Exact symbol rows are authoritative. Never fuzzy-match another symbol:
		// that was how FNiagaraStackGraphUtilities resolved to an unrelated header.
		TArray<FMonolithSourceSymbol> Exact = DB->GetSymbolsByName(Owner);
		int32 BestRank = MIN_int32;
		for (const FMonolithSourceSymbol& S : Exact)
		{
			const FString Path = DB->GetFilePath(S.FileId);
			if (!IsHeaderPath(Path)) continue;
			const int32 Rank = HeaderPathRank(Path);
			if (Rank > BestRank)
			{
				BestRank = Rank;
				OutFileId = S.FileId;
		...
		if (OutFileId != 0) return true;

		// Namespace-style utility owners (for example
		// FNiagaraEditorUtilities) often have no symbols row and may have thousands
		// of call sites, pushing their declaration header out of a broad FTS limit.
		// Try conventional owner/header suffixes first, but still require the file
		// to contain the actual namespace/class/struct/enum declaration.
		FString OwnerLeaf = Owner;
		const int32 NestedScope = OwnerLeaf.Find(TEXT("::"), ESearchCase::CaseSensitive, ESearchDir::FromEnd);
		if (NestedScope != INDEX_NONE) OwnerLeaf = OwnerLeaf.Mid(NestedScope + 2);
		TArray<FString> HeaderSuffixes;
		HeaderSuffixes.Add(OwnerLeaf + TEXT(".h"));
		if (OwnerLeaf.Len() > 1 && FString(TEXT("FUASIET")).Contains(FString::Chr(OwnerLeaf[0])))
		{
			HeaderSuffixes.Add(OwnerLeaf.Mid(1) + TEXT(".h"));
		}
		for (const FString& Suffix : HeaderSuffixes)
		{
			const TOptional<FMonolithSourceFile> Candidate = DB->FindFileBySuffix(Suffix);
			if (!Candidate.IsSet() || !IsHeaderPath(Candidate->Path)) continue;
			TArray<FString> Lines;
			if (!FFileHelper::LoadFileToStringArray(Lines, *Candidate->Path)) continue;
			for (const FString& Line : Lines)
			{
				if (!LineDeclaresOwner(Line, Owner)) continue;
				OutFileId = Candidate->Id;
				OutFilePath = Candidate->Path;
				return true;
			}
		}

		// Namespaces and non-reflected helper owners may have no symbols row. Use
		// source FTS only to collect candidate headers, then require an actual
		// class/struct/enum/namespace declaration in the file before accepting it.
		TArray<FMonolithSourceChunk> Chunks = DB->SearchSourceFTS(Owner, TEXT("all"), 256);
		TSet<int64> SeenFiles;
		for (const FMonolithSourceChunk& Chunk : Chunks)
		{
			if (SeenFiles.Contains(Chunk.FileId)) continue;
			SeenFiles.Add(Chunk.FileId);

			const FString Path = DB->GetFilePath(Chunk.FileId);
			if (!IsHeaderPath(Path)) continue;

			TArray<FString> Lines;
			if (!FFileHelper::LoadFileToStringArray(Lines, *Path)) continue;
			bool bDeclaresOwner = false;
			for (const FString& Line : Lines)
			{
				if (LineDeclaresOwner(Line, Owner)) { bDeclaresOwner = true; break; }
			}
			if (!bDeclaresOwner) continue;

			const int32 Rank = HeaderPathRank(Path);
			if (Rank > BestRank)
			{
				BestRank = Rank;
				OutFileId = Chunk.FileId;
				OutFilePath = Path;
			}
		}
		return OutFileId != 0;
	...
	static bool IsLikelyFunctionDeclaration(const FString& RawLine, const FString& Method,
		const FString& Owner, bool bHeader)
	{
		const FString Line = RawLine.TrimStartAndEnd();
		if (Line.IsEmpty() || Line.StartsWith(TEXT("//")) || Line.StartsWith(TEXT("/*"))
		...
		const FString Needle = Method + TEXT("(");
		const int32 MethodIdx = Line.Find(Needle, ESearchCase::CaseSensitive);
		if (MethodIdx == INDEX_NONE) return false;
		if (MethodIdx > 0 && IsIdentifierChar(Line[MethodIdx - 1])) return false;

		const FString Prefix = Line.Left(MethodIdx).TrimStartAndEnd();
		if (Prefix.Contains(TEXT("=")) || Prefix.Contains(TEXT("->")) || Prefix.EndsWith(TEXT("."))
			|| Prefix.StartsWith(TEXT("return ")) || Prefix.StartsWith(TEXT("if "))
			|| Prefix.StartsWith(TEXT("if(")) || Prefix.StartsWith(TEXT("for "))
			|| Prefix.StartsWith(TEXT("for(")) || Prefix.StartsWith(TEXT("while "))
			|| Prefix.StartsWith(TEXT("while(")) || Prefix.StartsWith(TEXT("switch "))
			|| Prefix.StartsWith(TEXT("switch(")) || Prefix.StartsWith(TEXT("ensure"))
			|| Prefix.StartsWith(TEXT("check")))
		{
			return false;
		}
		...
		if (!Owner.IsEmpty())
		{
			const FString Qualified = Owner + TEXT("::") + Needle;
			const int32 QualifiedIdx = Line.Find(Qualified, ESearchCase::CaseSensitive);
			if (QualifiedIdx != INDEX_NONE)
			{
				const FString BeforeOwner = Line.Left(QualifiedIdx).TrimStartAndEnd();
				// A bare Namespace::Method(...); line is a call, not a definition.
				return !BeforeOwner.IsEmpty() || Owner == Method;
			}

			// Inside the owning header the owner qualifier is normally omitted.
			if (!bHeader) return false;
			return !Prefix.IsEmpty() || Owner == Method;
		}

		// Free-function declarations need a return/type prefix. Plain class/struct
		// queries are handled from their exact symbol signature before this path.
		return !Prefix.IsEmpty();
	}

	static void AddSignatureUnique(TArray<FResolvedSignature>& Out, TSet<FString>& Seen,
		const FString& Signature, const FString& Source, const FString& FilePath, int32 Line, int32 Limit)
	{
		const FString Trimmed = Signature.TrimStartAndEnd();
		if (Trimmed.IsEmpty() || Seen.Contains(Trimmed) || Out.Num() >= Limit) return;
		Seen.Add(Trimmed);
		Out.Add({ Trimmed, Source, FilePath, Line });
	}

	static void CollectSignatures(FMonolithSourceDatabase* DB, const FString& Symbol, int32 Limit,
		TArray<FResolvedSignature>& Out)
	{
		Out.Reset();
		...
		TSet<FString> Seen;
		const int32 ScopeIdx = Symbol.Find(TEXT("::"), ESearchCase::CaseSensitive, ESearchDir::FromEnd);
		const FString Owner = ScopeIdx == INDEX_NONE ? FString() : Symbol.Left(ScopeIdx);
		const FString Method = ScopeIdx == INDEX_NONE ? Symbol : Symbol.Mid(ScopeIdx + 2);

		// Plain class/struct/enum symbols already carry the canonical declaration
		// signature. Return it instead of accidentally returning a constructor.
		if (ScopeIdx == INDEX_NONE)
		{
			TArray<FMonolithSourceSymbol> Exact = DB->GetSymbolsByName(Symbol);
			for (const FMonolithSourceSymbol& S : Exact)
			{
				if (S.Kind == TEXT("function") || S.Signature.IsEmpty()) continue;
				AddSignatureUnique(Out, Seen, S.Signature, TEXT("column"), DB->GetFilePath(S.FileId), S.LineStart, Limit);
			}
			if (Out.Num() > 0) return;
		}

		int64 HeaderFileId = 0;
		FString HeaderPath;
		const FString HeaderOwner = ScopeIdx == INDEX_NONE ? Symbol : Owner;
		if (ResolveHeaderFile(DB, HeaderOwner, HeaderFileId, HeaderPath))
		{
			TArray<FString> HeaderLines;
			if (FFileHelper::LoadFileToStringArray(HeaderLines, *HeaderPath))
			{
				if (ScopeIdx == INDEX_NONE)
				{
					// Namespace declaration fallback.
					for (int32 i = 0; i < HeaderLines.Num() && Out.Num() < Limit; ++i)
					{
						if (!LineDeclaresOwner(HeaderLines[i], Symbol)) continue;
						FString Decl = HeaderLines[i].TrimStartAndEnd();
						const int32 Brace = Decl.Find(TEXT("{"));
						if (Brace != INDEX_NONE) Decl = Decl.Left(Brace).TrimEnd();
						AddSignatureUnique(Out, Seen, Decl, TEXT("declaration_read"), HeaderPath, i + 1, Limit);
					}
				}
				else
				{
					for (int32 i = 0; i < HeaderLines.Num() && Out.Num() < Limit; ++i)
					{
						if (!IsLikelyFunctionDeclaration(HeaderLines[i], Method, Owner, /*bHeader=*/true)) continue;
						const FString Sig = FMonolithSourceActions::CompactDeclaration(HeaderLines, i);
						if (!Sig.Contains(Method + TEXT("("))) continue;
						AddSignatureUnique(Out, Seen, Sig, TEXT("declaration_read"), HeaderPath, i + 1, Limit);
					}
				}
			}
		}

		// Plain free-function fast path.
		if (ScopeIdx == INDEX_NONE)
		{
			TArray<FMonolithSourceSymbol> Functions = DB->GetSymbolsByName(Method, TEXT("function"));
			for (const FMonolithSourceSymbol& S : Functions)
			{
				if (Out.Num() >= Limit) break;
				if (S.Signature.IsEmpty() || S.Signature.Contains(TEXT("{")) || S.Signature.Contains(TEXT("\\"))) continue;
				AddSignatureUnique(Out, Seen, S.Signature, TEXT("column"), DB->GetFilePath(S.FileId), S.LineStart, Limit);
			}
		}

		// Add exact qualified definitions from source. Calls are rejected by the
		// declaration predicate, preventing return/assignment/bare call false positives.
		TArray<FMonolithSourceChunk> Chunks = DB->SearchSourceFTS(Symbol, TEXT("all"), 100);
		for (const FMonolithSourceChunk& Chunk : Chunks)
		{
			if (Out.Num() >= Limit) break;
			const FString FilePath = DB->GetFilePath(Chunk.FileId);
			TArray<FString> Lines;
			if (!FFileHelper::LoadFileToStringArray(Lines, *FilePath)) continue;
			const int32 WinStart = FMath::Max(0, Chunk.LineNumber - 1);
			const int32 WinEnd = FMath::Min(Lines.Num(), WinStart + 10);
			for (int32 i = WinStart; i < WinEnd && Out.Num() < Limit; ++i)
			{
				if (!IsLikelyFunctionDeclaration(Lines[i], Method, Owner, IsHeaderPath(FilePath))) continue;
				const FString Sig = FMonolithSourceActions::CompactDeclaration(Lines, i);
				if (!Sig.Contains(Method + TEXT("("))) continue;
				AddSignatureUnique(Out, Seen, Sig, TEXT("declaration_read"), FilePath, i + 1, Limit);
			}
		}
	}
}
...
bool FMonolithSourceActions::ResolveOwningModule(FMonolithSourceDatabase* DB, const FString& Symbol, FString& OutModule, FString& OutBuildCsNote)
{
	...
	int64 FileId = 0;
	FString FilePath;
	if (!MonolithSourceQueryFix::ResolveHeaderFile(DB, Symbol, FileId, FilePath)) return false;

	FString BuildCsPath;
	if (!DB->GetFileModuleInfo(FileId, OutModule, BuildCsPath))
	{
		return false;
	}
	...
	FString& OutInclude, bool& OutIncludable, FString& OutModule, FString& OutWarning)
{
	...
	FString BuildCsPath;
	DB->GetFileModuleInfo(FileId, OutModule, BuildCsPath);
	return true;
}
	...
	FString& OutSignature, FString& OutSource)
{
	...
	TArray<MonolithSourceQueryFix::FResolvedSignature> Signatures;
	MonolithSourceQueryFix::CollectSignatures(DB, Symbol, 1, Signatures);
	if (Signatures.Num() == 0) return false;
	OutSignature = Signatures[0].Signature;
	OutSource = Signatures[0].Source;
	return true;
}
...
bool FMonolithSourceActions::SymbolExists(FMonolithSourceDatabase* DB, const FString& Symbol)
{
	...
	int64 HeaderFileId = 0;
	FString HeaderPath;
	if (ScopeIdx == INDEX_NONE && MonolithSourceQueryFix::ResolveHeaderFile(DB, Symbol, HeaderFileId, HeaderPath))
	{
		return true;
	}
...
FMonolithActionResult FMonolithSourceActions::HandleGetIncludePath(const TSharedPtr<FJsonObject>& Params)
{
	...
	FString Include, ModuleName, Warning;
	if (!ResolveIncludeForSymbol(DB, Symbol, Include, bIncludable, ModuleName, Warning))
	{
		return FMonolithActionResult::Error(FString::Printf(TEXT("No symbol found matching '%s'."), *Symbol));
	}

	FString BuildCsNote;
	FString ResolvedModule;
	ResolveOwningModule(DB, Symbol, ResolvedModule, BuildCsNote);
	if (ModuleName.IsEmpty()) ModuleName = ResolvedModule;
...
FMonolithActionResult FMonolithSourceActions::HandleGetSignature(const TSharedPtr<FJsonObject>& Params)
{
	...
	TArray<MonolithSourceQueryFix::FResolvedSignature> Overloads;
	MonolithSourceQueryFix::CollectSignatures(DB, Symbol, Limit, Overloads);
...
FMonolithActionResult FMonolithSourceActions::HandleFindExampleUsage(const TSharedPtr<FJsonObject>& Params)
{
	...
	// Qualified methods match call syntax `Name(`. Plain class/struct symbols
	// match type references, while a plain namespace owner matches `Owner::`.
	FString MethodName = Symbol;
	const int32 ScopeIdx = Symbol.Find(TEXT("::"), ESearchCase::CaseSensitive, ESearchDir::FromEnd);
	if (ScopeIdx != INDEX_NONE) MethodName = Symbol.Mid(ScopeIdx + 2);
	const FString Owner = ScopeIdx == INDEX_NONE ? FString() : Symbol.Left(ScopeIdx);
	bool bPlainType = false;
	bool bPlainFunction = false;
	if (ScopeIdx == INDEX_NONE)
	{
		for (const FMonolithSourceSymbol& S : DB->GetSymbolsByName(Symbol))
		{
			if (S.Kind == TEXT("class") || S.Kind == TEXT("struct") || S.Kind == TEXT("enum"))
			{
				bPlainType = true;
				break;
			}
			if (S.Kind == TEXT("function")) bPlainFunction = true;
		}
	}
	int64 OwnerHeaderId = 0;
	FString OwnerHeaderPath;
	const bool bPlainOwner = ScopeIdx == INDEX_NONE && !bPlainType && !bPlainFunction
		&& MonolithSourceQueryFix::ResolveHeaderFile(DB, Symbol, OwnerHeaderId, OwnerHeaderPath);
	const FString NeedlePattern = ScopeIdx != INDEX_NONE ? MethodName + TEXT("(")
		: (bPlainType ? Symbol : (bPlainFunction ? MethodName + TEXT("(")
			: (bPlainOwner ? Symbol + TEXT("::") : MethodName + TEXT("("))));
	...
	for (const FMonolithSourceChunk& Chunk : Chunks)
	{
		...
		for (int32 i = WinStart; i < WinEnd; ++i)
		{
			...
			if (Trimmed.StartsWith(TEXT("//")) || Trimmed.StartsWith(TEXT("/*")) || Trimmed.StartsWith(TEXT("*"))) continue;

			// Example results are call/type-use sites, never the declaration/definition
			// itself. This is the key distinction get_signature previously missed.
			if (ScopeIdx != INDEX_NONE)
			{
				if (MonolithSourceQueryFix::IsLikelyFunctionDeclaration(
					L, MethodName, Owner, MonolithSourceQueryFix::IsHeaderPath(FilePath))) continue;
			}
			else if (bPlainFunction)
			{
				if (MonolithSourceQueryFix::IsLikelyFunctionDeclaration(
					L, MethodName, FString(), MonolithSourceQueryFix::IsHeaderPath(FilePath))) continue;
			}
			else if (MonolithSourceQueryFix::LineDeclaresOwner(L, Symbol))
			{
				continue;
			}
			...
			// Engine source includes both Engine/Source and Engine/Plugins. Treating
			// Engine Plugins as project made --prefer=engine rank sandbox Monolith
			// calls ahead of canonical NiagaraEditor examples.
			FString NormPath = FilePath; NormPath.ReplaceInline(TEXT("\\"), TEXT("/"));
			const bool bIsEngine = NormPath.Contains(TEXT("Engine/Source/"))
				|| NormPath.Contains(TEXT("Engine/Plugins/"));
